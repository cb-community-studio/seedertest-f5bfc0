...
import seedersFakerFactory from "./seedersFakerFactory";
...
const SeedersFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = seedersFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("seeders").create(elem));
...
export default connect(null, mapDispatch)(SeedersFakerDialogComponent);
